#!/usr/bin/env sh
# 🅳🅾🆃🅵🅸🅻🅴🆂 (v0.2.450) - History configuration.
...
setopt hist_ignore_dups         # Prevents storing only repeated lines that are adjacent to one another
setopt hist_ignore_all_dups     # Removes old duplicates of the line and keeps the new one even if lines are not adjacent
setopt hist_find_no_dups        # Even if duplicates are saved in the history, they won’t be shown more than once while cycling back.

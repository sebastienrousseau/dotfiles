...
if bash -n "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: core.sh has valid syntax"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: core.sh has syntax errors"
fi
...
if grep -q "cmd_apply\|dot_apply" "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: defines apply command"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should define apply command"
fi

test_start "core_defines_cmd_sync"
if grep -q "cmd_sync\|dot_sync" "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: defines sync command"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should define sync command"
fi

test_start "core_defines_cmd_update"
if grep -q "cmd_update\|dot_update" "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: defines update command"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should define update command"
fi

test_start "core_defines_cmd_diff"
if grep -q "cmd_diff\|dot_diff" "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: defines diff command"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should define diff command"
fi

test_start "core_defines_cmd_status"
if grep -q "cmd_status\|dot_status" "$CORE_FILE" 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: defines status command"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should define status command"
fi
...
if grep -qE '"/home/[a-z]+' "$CORE_FILE" 2>/dev/null; then
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should not have hardcoded home paths"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: no hardcoded home paths"
fi
...
if grep -qE 'set -[euo]|errexit|nounset' "$CORE_FILE" 2>/dev/null || \
  ...
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should have error handling"
fi
...
if grep -qE 'rm -rf /[^$]|rm -rf ~' "$CORE_FILE" 2>/dev/null; then
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: has dangerous unguarded rm -rf"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: no dangerous unguarded rm commands"
fi

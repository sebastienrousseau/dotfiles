...
# Tests for 05-core-safety.sh.tmpl template structure

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="${REPO_ROOT:-$(cd "$SCRIPT_DIR/../../.." && pwd)}"
source "$SCRIPT_DIR/../framework/assertions.sh"

TMPL_FILE="$REPO_ROOT/dot_config/shell/05-core-safety.sh.tmpl"

test_start "core_safety_exists"
assert_file_exists "$TMPL_FILE" "05-core-safety.sh.tmpl should exist"

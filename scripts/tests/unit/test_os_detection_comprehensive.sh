...
if [[ -n "${OS:-}" && -n "${ARCH:-}" && -n "${target_os:-}" ]]; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: OS/ARCH/target_os are set"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: detect_os did not set expected variables"
fi

test_start "target_os_is_known"
if [[ "$target_os" =~ ^(macos|wsl2|debian|fedora|arch|linux|unknown)$ ]]; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: target_os=$target_os"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: unexpected target_os=$target_os"
fi

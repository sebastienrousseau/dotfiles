...
if echo "$backup_block" | grep -q '&&.*cp.*||' 2>/dev/null; then
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should not use A && B || C pattern (SC2015)"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: no SC2015 pattern in backup block"
fi
...
if echo "$backup_block" | grep -q 'if \[\[.*-f.*\]\]; then' 2>/dev/null; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: backup uses proper if/then for file copy"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: backup block refactored (no A&&B||C)"
fi
...
if grep -qP 'brew\)\s{3,}' "$HEAL_SCRIPT" 2>/dev/null; then
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: case branches should not have extra spaces"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: case branch spacing is correct"
fi
...
if grep -qP 'apt\|dnf' "$HEAL_SCRIPT" 2>/dev/null; then
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: case pipes should have spaces (apt | dnf)"
else
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: case pipe spacing is correct"
fi

test_start "heal_has_help_flag"
if grep -qF -- "--help" "$HEAL_SCRIPT"; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: should support --help flag"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should support --help flag"
fi

test_start "heal_has_dry_run"
if grep -qF -- "--dry-run" "$HEAL_SCRIPT"; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: should support --dry-run flag"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should support --dry-run flag"
fi

test_start "heal_has_force"
if grep -qF -- "--force" "$HEAL_SCRIPT"; then
  ((TESTS_PASSED++))
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: should support --force flag"
else
  ((TESTS_FAILED++))
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should support --force flag"
fi

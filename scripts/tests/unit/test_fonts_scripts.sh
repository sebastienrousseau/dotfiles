...
if [[ -f "$FONTS_DIR/install-nerd-fonts.sh" ]]; then
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: install-nerd-fonts.sh exists"
else
  ((TESTS_FAILED++)) || true
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: install-nerd-fonts.sh should exist"
fi
...
if [[ -f "$FONTS_DIR/install-nerd-fonts.sh" ]] && grep -qE 'nerd-fonts|ryanoasis' "$FONTS_DIR/install-nerd-fonts.sh" 2>/dev/null; then
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: uses official nerd fonts source"
else
  ((TESTS_FAILED++)) || true
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should use official source"
fi
...
if [[ -f "$FONTS_DIR/install-nerd-fonts.sh" ]] && grep -qE 'fonts|\.local/share/fonts|Library/Fonts' "$FONTS_DIR/install-nerd-fonts.sh" 2>/dev/null; then
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: installs to proper font directory"
else
  ((TESTS_FAILED++)) || true
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: should use proper font directory"
fi

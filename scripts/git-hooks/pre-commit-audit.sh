...
# Exclude assignments to values containing common placeholder strings
SENSITIVE_PATTERNS="(GITHUB_TOKEN|API_KEY|SECRET|password|private_key|AKIA[0-9A-Z]{16})[[:space:]]*[:=]"
if echo "$STAGED_FILES" | xargs grep -EiE "$SENSITIVE_PATTERNS" /dev/null | grep -vE "test_|assertions.sh|.pre-commit|README|CHANGELOG|FEATURES.md|CONFIG_STRATEGY.md|docs/" | grep -viE "dummy|placeholder|example|test|sk-placeholder" >/dev/null 2>&1; then
  echo -e "${RED}FAILED${NC}"
  echo -e "      ${YELLOW}⚠ Potential secret detected in staged files:${NC}"
  echo "$STAGED_FILES" | xargs grep -EiE "$SENSITIVE_PATTERNS" /dev/null | grep -vE "test_|assertions.sh|.pre-commit|README|CHANGELOG|FEATURES.md|CONFIG_STRATEGY.md|docs/" | grep -viE "dummy|placeholder|example|test|sk-placeholder" | sed 's/^/      /'
  FAILED=1
else
...
# Exclude test files, documentation, the hook itself, and Nix files
GHOST_EXCLUDES="test_|assertions.sh|pre-commit|interop.md|WSL2_NIX_TROUBLESHOOTING.md|\.nix"
...
if command -v shellcheck >/dev/null 2>&1; then
  ...
  if [[ -n "$SHELL_FILES" ]]; then
    if ! echo "$SHELL_FILES" | xargs shellcheck -x --severity=error -e SC1091 >/dev/null 2>&1; then
      echo -e "${RED}FAILED${NC}"
      echo -e "      ${YELLOW}⚠ Shell syntax errors detected:${NC}"
      echo "$SHELL_FILES" | xargs shellcheck -x --severity=error -e SC1091 | sed 's/^/      /'
      FAILED=1
    else
      echo -e "${GREEN}PASSED${NC}"
    fi
...
if [[ -n "$OFFENDING_WHITESPACE" ]]; then
  ...
  echo "$OFFENDING_WHITESPACE" | sed 's/^/      /'
  FAILED=1

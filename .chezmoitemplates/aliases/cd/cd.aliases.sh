...
#-----------------------------------------------------------------------------
export DOTFILES_VERSION="0.2.497"
...
[[ -d "${DOTF_DIR}" ]] && alias dotd='cd_with_history "${DOTF_DIR}"'  # Dotfiles dir
...
# NOTE: Stack aliases (dirs, pd, popd) → cd-stack.aliases.sh
# NOTE: Shorthand aliases (mcd, bm*, dh, pr, ld) → cd-init.aliases.sh
# NOTE: Init logic (RESTORE_LAST_DIR) → cd-init.aliases.sh
# NOTE: Help functions and aliases → cd-help.aliases.sh

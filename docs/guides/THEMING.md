...
Each session hashes its name into twelve shades derived from the active theme's primary, secondary, and tertiary colors. Collision probing keeps the active session set visually distinct; renaming a session updates that identity automatically. Terminals that support focus events dim inactive status blocks; CWD and other-session hints appear only on wide clients to avoid crowding narrow AI panes.

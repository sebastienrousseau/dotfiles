...
| **PowerShell** | **7.4 LTS** | The Windows parity surface. No feature needs 7.5. 7.4 LTS retires on 2026-11-10, when the floor moves to 7.5. | `reliability-gate.yml` job `PowerShell Contract` and the `ci.yml` Windows job, on `windows-latest` (7.4 LTS) |

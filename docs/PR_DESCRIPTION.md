...
### Architecture
v0.2.475 is not just "dotfiles" but a portable **Shell Distribution** managed by `chezmoi` ("source of truth" in `~/.dotfiles`).
...
- **Supply Chain Safety**:
  - **Pinned Install**: Installation commands are pinned to the specific release tag (`v0.2.475`) to prevent drift.

...
### Architecture
v0.2.492 is a portable **shell distribution** managed by `chezmoi` (source of truth in `~/.dotfiles`).
...
- **Supply chain safety**:
  - **Pinned install**: Installation commands pin to the specific release tag (`v0.2.492`) to prevent drift.
...
### Verification
- **Performance**: Run `scripts/benchmark.sh` to measure startup time on your machine. Use `DOTFILES_FAST=1` for the quickest first prompt.
...
```bash
sh -c "$(curl -fsSL https://raw.githubusercontent.com/sebastienrousseau/dotfiles/v0.2.492/install.sh)"
```

...
### Architecture
v0.2.493 is a portable **shell distribution** managed by `chezmoi` (source of truth in `~/.dotfiles`).
...
- **Supply chain safety**:
  - **Pinned install**: Installation commands pin to the specific release tag (`v0.2.493`) to prevent drift.
...
```bash
sh -c "$(curl -fsSL https://raw.githubusercontent.com/sebastienrousseau/dotfiles/v0.2.493/install.sh)"
```

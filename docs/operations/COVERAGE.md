...
| Surface | What runs |
|---|---|
| **PR + push to main** | `.github/workflows/coverage.yml` → `Coverage / kcov` job → uploads lcov.info to Codecov and fails the build below `MIN_COVERAGE_PCT` (currently `39`, ratcheted up each slice). |
...
`MIN_COVERAGE_PCT=39` in `.github/workflows/coverage.yml`. Slice 1
...
value sits at **39.82%** (`4925/12369` lines) after the first core
coverage-ratchet slice drove `lib/dot/ui.sh` from 37.61% to 52.23%,
building on the #954 deep-branch pass for `scripts/theme/switch.sh`,

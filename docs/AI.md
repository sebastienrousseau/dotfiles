...
**Security:** the server binds to loopback by default. If you set `DOT_AI_HOST` to a LAN address, both the launcher *and* the server itself refuse to start unless `DOT_AI_API_KEY` is set — an unprotected network proxy leaks your subscription. Every request needs a key: without `DOT_AI_API_KEY` the server and `dot ai proxy local on` share a random 0600 token at `$XDG_STATE_HOME/dotfiles/ai-serve/gateway.token` (print it with `dot-ai-serve --print-token`). Requests whose `Host` header is not the server's own address are refused (DNS-rebinding guard), and the `claude` engine runs with all tools disabled in an empty temporary directory.

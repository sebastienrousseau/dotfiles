...
#
# Example: Workstation environment manifest
#
# Render the v1 environment manifest that SBOM and compliance tooling consumes.
#
# Backing module: scripts/dot/commands/env-emit.sh
...
printf '=== Workstation environment manifest ===\n\n'
...
printf '\n--- live output ---\n\n'
printf "  dot env emit                  # canonical JSON\n"
printf "  dot env emit --format ndjson  # one object per tool\n"
printf "  dot env emit --output env.json\n"
bash "$dot" env emit --compact 2>&1 | head -3 || true

printf '\nWorkstation environment manifest example complete.\n'

...
#
# Example: Fleet status, drift and apply
#
# Treat a set of machines as one fleet: status, drift history, RBAC, SSH apply.
#
# Backing module: scripts/dot/commands/fleet.sh
...
printf '=== Fleet status, drift and apply ===\n\n'
...
printf '\n--- live output ---\n\n'
bash "$dot" fleet status --json || true
bash "$dot" fleet drift | head -5 || true

printf '\nFleet status, drift and apply example complete.\n'

...
| `aur/PKGBUILD` | template | Arch User Repository package definition. Bumped per release by `.github/workflows/release-distribute-aur.yml` (push to `ssh://aur@aur.archlinux.org/dot-cli-git.git`). First-time publication is automatic: the workflow inits a local repo and pushes — AUR creates the slot if the name is free, or rejects with `permission denied: <user>` if it's already taken. |
...
4. **`release-distribute-*.yml`** fan out:
   ...
   - AUR: direct push to `ssh://aur@aur.archlinux.org/dot-cli-git.git` (requires `AUR_SSH_KEY` secret; the registered SSH key must belong to the package's maintainer account, otherwise the push is rejected as `permission denied: <user>`).

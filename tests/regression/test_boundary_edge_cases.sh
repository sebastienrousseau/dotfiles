...
# Why: Edge-case input boundary regressions (empty strings, max sizes, unusual paths, off-by-one indexing).
...
assert_file_exists "$REPO_ROOT/defaults/.chezmoitemplates/paths/00-default.paths.sh" "default paths file must exist"

...
DATA_FILE="$REPO_ROOT/defaults/.chezmoidata.toml"
ZSHRC_TMPL="$REPO_ROOT/defaults/dot_config/zsh/dot_zshrc.tmpl"
SAFETY_TMPL="$REPO_ROOT/defaults/dot_config/shell/05-core-safety.sh"
...
DOCKER_ALIASES="$REPO_ROOT/defaults/.chezmoitemplates/aliases/docker/docker.aliases.sh"
GIT_ALIASES="$REPO_ROOT/defaults/.chezmoitemplates/aliases/git/git.aliases.sh"
...
if [[ -f "$CHEZMOI_LIB" ]]; then
  ...
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: skipped (chezmoi.sh not found)"
fi

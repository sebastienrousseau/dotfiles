...
PATHS_DIR="$REPO_ROOT/defaults/.chezmoitemplates/paths"
...
if [[ -f "$PATHS_DIR/00-default.paths.sh" ]]; then
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: default paths exists"
else
  ((TESTS_FAILED++)) || true
  printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: default paths should exist"
fi
...
if [[ "$invalid" -eq 0 ]]; then
  ((TESTS_PASSED++)) || true
  printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: all paths files valid"
else
  ((TESTS_FAILED++)) || true
...
# Allow linuxbrew but reject other /home/user or /Users/user patterns
if grep -rqE '"/home/(linuxbrew)?[^$]|/Users/[^$]' "$PATHS_DIR"/*.sh | grep -v "linuxbrew" 2>/dev/null; then
  ((TESTS_FAILED++)) || true

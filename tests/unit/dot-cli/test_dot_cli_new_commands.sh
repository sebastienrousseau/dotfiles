...
test_start "defaults/dot_cli_has_verify"
assert_file_contains "$DOT_CLI" "verify" "dot CLI should handle verify command"

test_start "defaults/dot_cli_has_scorecard"
assert_file_contains "$DOT_CLI" "scorecard" "dot CLI should handle scorecard/score command"

test_start "defaults/dot_cli_has_snapshot"
assert_file_contains "$DOT_CLI" "snapshot" "dot CLI should handle snapshot command"

test_start "defaults/dot_cli_has_heal"
assert_file_contains "$DOT_CLI" "heal" "dot CLI should handle heal command"

test_start "defaults/dot_cli_has_update_alias"
assert_file_contains "$DOT_CLI" "update" "dot CLI should handle update as hidden alias"

# --- Wave 1: _require_platform defined ---

test_start "defaults/dot_cli_has_require_platform"
...
test_start "defaults/dot_cli_doctor_dispatches_unified"
DIAG_CMD="$REPO_ROOT/scripts/dot/commands/diagnostics.sh"
assert_file_contains "$DIAG_CMD" "doctor-unified.sh" "doctor should dispatch to doctor-unified.sh"

# --- Wave 3: sync with flags + env command ---

test_start "defaults/dot_cli_has_sync"
assert_file_contains "$DOT_CLI" "sync" "dot CLI should handle sync command"

test_start "defaults/dot_cli_sync_pull_flag"
assert_file_contains "$DOT_CLI" "--pull" "sync should support --pull flag"

test_start "defaults/dot_cli_sync_check_flag"
assert_file_contains "$DOT_CLI" "--check" "sync should support --check flag"

test_start "defaults/dot_cli_has_env"
assert_file_contains "$DOT_CLI" 'env' "dot CLI should handle env command"

# --- Wave 4: profile + keys sign-check ---

test_start "defaults/dot_cli_has_profile"
assert_file_contains "$DOT_CLI" "profile" "dot CLI should handle profile command"

test_start "defaults/dot_cli_keys_sign_check"

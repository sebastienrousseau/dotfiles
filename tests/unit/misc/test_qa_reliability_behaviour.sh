...
if LOG_FILE="$log_file" bash "$stub_repo/scripts/qa/reliability-audit.sh" --quick >/dev/null 2>&1; then
  if grep -q "test_runner:" "$log_file" && ! grep -q "test_runner:-i" "$log_file"; then
    ((TESTS_PASSED++))
    printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: quick mode runs unit path only"
  else
    ((TESTS_FAILED++))
    printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: quick mode should skip integration runner"
  fi
else
  ((TESTS_FAILED++))
...
if LOG_FILE="$log_file" bash "$stub_repo/scripts/qa/reliability-audit.sh" --with-integration >/dev/null 2>&1; then
  if grep -q "test_runner:-i" "$log_file"; then
    ((TESTS_PASSED++))
    printf '%b\n' "  ${GREEN}✓${NC} $CURRENT_TEST: integration flag runs integration suite"
  else
    ((TESTS_FAILED++))
    printf '%b\n' "  ${RED}✗${NC} $CURRENT_TEST: integration flag should run integration suite"
  fi
else
  ((TESTS_FAILED++))
